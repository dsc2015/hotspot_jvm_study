...
// knows about ObjectWaiters, so we'll have to reconcile(顺从) that code.
// See next_waiter(), first_waiter(), etc.
//只在栈上分配的对象，这个是可以理解的，因为监视器对象只有与线程关联，也就是存在方法的调用链的时候
...
// WARNING:
//   This is a very sensitive(敏感的) and fragile(脆弱的) class. DO NOT make any
...
// heavy weight lock due to contention //由于竞争，从轻量级锁转化为重量级锁的过程
